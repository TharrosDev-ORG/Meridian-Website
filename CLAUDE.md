...
7. **Mobile Isolation Strategy**: Desktop-specific optimizations (hovers, spacing, scaling, readability caps) must be strictly encapsulated in `@media (min-width: 1101px)` to protect mobile stability. The site ships a dedicated **Desktop Optimizations v1.5** block in `globals.css` plus per-page desktop blocks in every `pageCss.ts`; all additions belong inside that gate.
...
9. **Routes outside `(site)` must self-wrap `TransitionWrapper`.** `/register` is the only current example. If you add another route outside the group (or a new top-level group), wrap its page content in `<TransitionWrapper>` so the `.page-sweep` entry animation still fires.
...
- RESET · AESTHETICS · SCROLLBAR · **DESIGN TOKENS** · BASE · FOCUS VISIBLE · BACK-TO-TOP · KEYFRAMES · MARQUEE · REDUCED MOTION · MOBILE BASE · **NAV BAR** · HAMBURGER · **MOBILE DRAWER** · MOBILE RESPONSIVE · PAGE TRANSITIONS · STAGGERED REVEALS · SUCCESS STATE UTILS · SHARED PAGE UTILS · **SHARED MODULES (v1.3)** · **DESKTOP OPTIMIZATIONS (v1.5)** · **FOOTER** · SHARED RESPONSIVE OVERRIDES · Responsive Footer · REGISTER SECTION · SOCIAL RECORD · MOBILE UI POLISH.

### Desktop Optimizations (v1.5)
A dedicated `@media (min-width: 1101px)` block in `globals.css` carries site-wide desktop polish: paragraph readability caps (`hero-sub`, `register-body`, `social-record-copy`, `module-intro-copy`), unified page-hero padding + `min-height: 64vh`, 16ch title wrap, denser nav (`padding: 0 64px`, comfortable link padding), richer module-card hover layering, primary-button elevation, footer link hover-lift, larger `.arc-btn`, and an ultra-wide `@media (min-width: 1600px)` tier for additional breathing room. Per-page `pageCss.ts` files mirror the same pattern with their own `@media (min-width: 1101px)` blocks for page-scoped concerns (stats bar, team card photo size, benefits/FAQ padding, gathering/sig-card hover, meta-row hover shift, etc.). **All desktop rules stay inside the 1101px gate — never leak into mobile.**
